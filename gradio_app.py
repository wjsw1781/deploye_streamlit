...
css = ".limit-height-container {height: 400px; overflow: auto !important}"
with gr.Blocks(fill_height=True,css=css) as demo:
    ...
    with gr.Tab(label='大型纪录片的设计-一切从topic开始'):
        ...
        with gr.Row():
            ...
            group_info=gr.Json(label='当前表格的元数据信息',elem_classes='limit-height-container')
            key_word=gr.Textbox(label='输入关键词',value='',placeholder='通过关键词搜索新增条目')
            steps=gr.CheckboxGroup(label='所有数据状态情况',choices=[],elem_classes='limit-height-container')
            search_btn=gr.Button(value='搜索')

        with gr.Row(equal_height=True):
            search_html=gr.HTML("<div>网页预览占位符</div>",label='b站检索结果',elem_classes='limit-height-container')
            search_bvids_df=gr.Dataframe(label='检索到的bvid',height=700,scale=1,elem_classes='limit-height-container')
            current_item=gr.JSON(value={},label='一个json表示当前选中的条目',elem_classes='limit-height-container')
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    one_ok=gr.Button(value='只修改这一个',interactive=True)
        ...
        def scrapy_bvids_by_search_html(group,key_word,current_item):
            table_name=get_pinyin(group)

            item={**current_item,'group':group,'key_word':key_word,}
